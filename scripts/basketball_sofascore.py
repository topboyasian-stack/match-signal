...
def odds(eid):
    ...
    for name,choices in markets(p):
        n=name.lower();parsed=[(str(c.get("name") or ""),prob(c.get("decimalValue"))) for c in choices];parsed=[x for x in parsed if x[1]]
        if r["total"] is None and any(k in n for k in ("total","over/under","over under")):
            rows=[]
            for label,pv in parsed:
                m=re.search(r"(over|under)\s*([0-9]+(?:\.[0-9])?)",label,re.I)
                if m:rows.append((m.group(1).lower(),float(m.group(2)),pv))
            for line in sorted({x[1] for x in rows}):
                ov=next((pv for side,l,pv in rows if side=="over" and l==line),None);un=next((pv for side,l,pv in rows if side=="under" and l==line),None)
                if ov and un:
        ...
        if r["moneyline"] is None and ("winner" in n or "moneyline" in n or n in {"1x2","full time"}):
            h=next((pv for label,pv in parsed if label.lower() in {"1","home"}),None);a=next((pv for label,pv in parsed if label.lower() in {"2","away"}),None)
            if h and a:
...
def settle(history):
    for p in history:
        ...
        try:
            ...
            if line is not None:
                total=h+a;p["actual_total"]=total;p["actual_total_pick"]="over" if total>line else "under" if total<line else "push";chosen=p.get("markets",{}).get("total_ou",{}).get("pick");p["total_correct"]=chosen in {"over","under"} and p["actual_total_pick"]==chosen
