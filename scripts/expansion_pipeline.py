...
def nba_fallback():
 errs=[]
 try:
  r=S.get(NBA_PARQUET,timeout=60);r.raise_for_status()
  ...
  def col(*names):
   ...
   return None
  date_c=col('game_date','date','game_date_time');gid_c=col('game_id','gameid','id')
  home_c=col('home_display_name','home_team','home_team_name');away_c=col('away_display_name','away_team','away_team_name')
  hs_c=col('home_score','home_team_score','pts_home','home_pts');as_c=col('away_score','away_team_score','pts_away','away_pts')
  rows=[]
  if date_c and home_c and away_c and hs_c and as_c:
   for _,r0 in df.iterrows():
    ...
    gid=str(r0.get(gid_c)) if gid_c else f'nba|{d.date()}|{h}|{a}'
    rows.append({'event_id':gid,'date':d.isoformat(),'home':h,'away':a,'home_id':'','away_id':'','home_score':hs,'away_score':as_,'markets':{},'source':NBA_PARQUET})
  else:
  ...
  errs.append(f'{NBA_PARQUET}: zero usable rows')
 except Exception as e:errs.append(f'{NBA_PARQUET}: {e}')
 return [],errs
def collect(name):
