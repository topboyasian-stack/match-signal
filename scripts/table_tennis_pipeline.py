...
def fetch_sportybet():
    r=requests.get(SPORTYBET,params={"sportId":"sr:sport:20","pageSize":100,"pageNum":1,"todayGames":"false","timeline":168,"_t":int(time.time()*1000)},headers=SPORTYBET_HEADERS,timeout=25)
...
def main():
    ...
    save(STATUS,{"updated_at":datetime.now(timezone.utc).isoformat(),"sport":"table_tennis","sport_id":"sr:sport:20","history_events":len(history),"upcoming_events":len(enriched),"candidate_count":len(candidates),"model_status":"PROMOTED" if promoted else ("TESTING" if history else "COLLECTING"),"source_results":"TheSportsDB public event results","source_odds":"SportyBet NG","source_errors":source_errors})
