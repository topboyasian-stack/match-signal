...
def row_from_prediction(p):
    ...
    if odds is not None and p.get("settled"):pl=round(odds-1,6) if p.get("correct") is True else (-1.0 if p.get("correct") is False else None)
...
def main():
    ...
    monthly=Counter((r["published_at"][:7] for r in rows));recent_months=sorted(monthly)[-6:]
    tip_ready=all(monthly[m]>=30 for m in recent_months) and len(recent_months)>=6
    ledger={"version":"verified-tip-ledger-v1","updated_at":datetime.now(timezone.utc).isoformat(),"status":"PAPER_TRACKED_NOT_LIVE_MONETIZED","methodology":{"publication_rule":"calculated_at must precede start_time","settlement_source":"prediction_history.json","roi_rule":"ROI only when a real bookmaker decimal price exists","no_retroactive_publication":True},"summary":aggregate(rows),"by_sport":by_sport,"by_market":by_market,"by_confidence":by_conf,"monthly_publication_counts":dict(sorted(monthly.items())),"tippingsports_readiness":{"six_month_public_history":len(recent_months)>=6,"minimum_30_tips_each_recent_month":tip_ready,"note":"This is only a data-volume gate; TippingSports also requires its own active account history, profitability/consistency and application approval."},"rows":rows}
    save("verified_tip_ledger.json",ledger)
    ...
    save("tippingsports_export.json",{"version":"tippingsports-prep-v1","status":"EXPORT_READY_MANUAL_SUBMISSION","readiness":ledger["tippingsports_readiness"],"note":"External platform eligibility/submission remains separate; this is a structured paper-tip export.","tips":tips})
    ...
    print(f"Verified ledger: {len(rows)} published paper rows | TippingSports volume gate={tip_ready}")
