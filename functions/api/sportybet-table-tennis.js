...
export async function onRequestGet(context){
  ...
  try{
    const feed=await fetchSportFeed({sportId:'sr:sport:20',pageSize,pageNum,todayGames:'false',timeline,_t:Date.now()});
    const events=[];
    ...
    return new Response(JSON.stringify({ok:true,status:'UPSTREAM_UNAVAILABLE',updated_at:new Date().toISOString(),events_count:0,events:[],error:String(e?.message||e),provider:'SportyBet NG',sport_id:'sr:sport:20'}),{status:200,headers:headers()});
  }
}