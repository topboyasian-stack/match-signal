(function () {
  ...
  function state(v) {
    var d = dateOf(v); if (!d) return { label:'Status', text:'Start time unavailable', cls:'unknown' };
  ...
  function timer(v) { var c=state(v); return '<div class="timer '+c.cls+'" data-start="'+esc(v||'')+'"><span class="timerLabel">'+esc(c.label)+'</span><strong class="timerValue">'+esc(c.text)+'</strong></div>'; }
  function updateTimers() {
    document.querySelectorAll('#oddsBuilder .timer[data-start]').forEach(function(el){ var c=state(el.getAttribute('data-start')); el.className='timer '+c.cls; var a=el.querySelector('.timerLabel'),b=el.querySelector('.timerValue'); if(a)a.textContent=c.label; if(b)b.textContent=c.text; });
  }
  function render(data) {
    ...
    var cards=legs.map(function(l,i){return '<article class="card"><div class="meta"><span>Leg '+(i+1)+' · '+esc(String(l.sport||'').toUpperCase())+' · '+esc(l.competition||'—')+'</span><span>Fair odds '+esc(l.model_fair_odds==null?'—':l.model_fair_odds)+'</span></div><div class="teams">'+esc(l.match||'—')+'</div><div class="pick">Selection: <b>'+esc(l.pick||'—')+'</b><span class="conf">Model '+pct(l.model_probability)+'</span></div>'+timer(l.start_time)+'<div class="startTime">Start: '+esc(local(l.start_time))+' <span>· your browser time</span></div><div class="section"><div class="row"><span>Market</span><b>'+esc(l.market||'—')+'</b></div><div class="row"><span>Model fair odds</span><b>'+esc(l.model_fair_odds==null?'—':l.model_fair_odds)+'</b></div><div class="row"><span>Bookmaker odds</span><b>Verify manually</b></div></div></article>';}).join('');
  ...
  function load(){var target=document.getElementById('oddsBuilder');if(!target)return;target.innerHTML='<div class="empty">Loading Odds Builder data…</div>';fetch('./data/odds_builder.json?v=20260916-odds-final-'+Date.now(),{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('odds_builder.json HTTP '+r.status);return r.json();}).then(render).catch(function(e){target.innerHTML='<div class="empty">Accumulator data unavailable: '+esc(e.message)+'</div>';});}
  function boot(){load();window.setInterval(load,60000);window.setInterval(updateTimers,1000);}
