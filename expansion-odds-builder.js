(function () {
  ...
  function countdownText(v) {
    var d = parseStart(v);
    if (!d) return { text: 'Start time unavailable', state: 'unknown' };
    var diff = d.getTime() - Date.now();
    if (diff <= 0) return { text: 'STARTED', state: 'started' };
    ...
    return { text: text, state: diff <= 15 * 60 * 1000 ? 'soon' : 'upcoming' };
  }
  ...
  function timerMarkup(v) {
    var c = countdownText(v);
    return '<div class="timer ' + c.state + '" data-start="' + esc(v || '') + '"><span class="timerLabel">' + (c.state === 'started' ? 'Status' : 'Starts in') + '</span><strong class="timerValue">' + esc(c.text) + '</strong></div>';
  }
  function load() {
  ...
  function render(x) {
    ...
    var cards = legs.map(function (l, i) {
      ...
      return '<div class="card"><div class="meta"><span>Leg ' + (i + 1) + ' · ' + esc(sport.toUpperCase()) + ' · ' + esc(competition) + '</span><span>Fair odds ' + esc(l.model_fair_odds) + '</span></div><div class="teams">' + esc(l.match) + '</div><div class="pick">Selection: <b>' + esc(l.pick) + '</b><span class="conf">Model ' + pct(l.model_probability) + '</span></div>' + timerMarkup(l.start_time) + '<div class="startTime">Kickoff / start: ' + esc(localStart(l.start_time)) + ' <span>· your browser time</span></div><div class="section"><div class="row"><span>Market</span><b>' + esc(l.market || '—') + '</b></div><div class="row"><span>Model fair odds</span><b>' + esc(l.model_fair_odds) + '</b></div><div class="row"><span>Actual bookmaker odds</span><b>Verify manually</b></div></div></div>';
    }).join('');
  ...
  function updateTimers() {
    document.querySelectorAll('#oddsBuilder .timer[data-start]').forEach(function (el) {
      var c = countdownText(el.getAttribute('data-start'));
      el.className = 'timer ' + c.state;
      var label = el.querySelector('.timerLabel');
      var value = el.querySelector('.timerValue');
      if (label) label.textContent = c.state === 'started' ? 'Status' : 'Starts in';
