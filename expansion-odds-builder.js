(function () {
  ...
  function players(match) { var parts = String(match || '').split(/\s+vs\.?\s+/i); return { p1: (parts[0] || '').trim(), p2: (parts[1] || '').trim() }; }
  function selectionInfo(leg) {
    var ps = players(leg.match); var rawPick = String(leg.pick || '').trim(); var pick = rawPick.toLowerCase();
    var positionMatch = pick.match(/(?:—|-|:)\s*(p1|p2)\s*$/i); var position = positionMatch ? positionMatch[1].toLowerCase() : pick;
    if ((leg.market === 'winner' || leg.market === 'games_handicap') && (position === 'p1' || position === 'p2')) {
      var pos = position.toUpperCase(); var player = position === 'p1' ? ps.p1 : ps.p2; return { position: pos, player: player, text: pos + ' · ' + player };
    }
    return { position: '', player: '', text: rawPick.replace(/^.*?—\s*/, '') || '—' };
  }
  function formatElapsed(diff) {
    var total = Math.max(0, Math.floor(diff / 1000)); var days = Math.floor(total / 86400); total %= 86400;
  ...
  function localStart(v) { var d = parseStart(v); if (!d) return '—'; return d.toLocaleString([], { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }); }
  function settlementFromRecord(r) {
    if (!r || r.settled !== true) return null;
    var pick = String(r.pick || '').toLowerCase(); var actual = String(r.actual || '').toLowerCase(); var correct = null;
    if (r.market === 'total_games' || r.market === 'total_goals' || r.market === 'over_under') {
      var markets = r.actual_markets || {}; actual = String(markets.total_games_result || markets.total_goals_result || markets.over_under_result || r.actual || '').toLowerCase(); if (actual) correct = pick === actual;
    } else if (pick && actual) correct = pick === actual;
    if (correct === null) return null; return { settled: true, correct: !!correct, actual: r.actual || actual || '—', settledAt: r.settled_at };
  }
  var settlementMap = {};
  ...
  function loadSettlements() {
    return Promise.all([loadOne('tennis_prediction_archive.json'), loadOne('prediction_history.json')]).then(function (sets) {
      settlementMap = {};
      sets.forEach(function (rows) { (Array.isArray(rows) ? rows : []).forEach(function (r) {
        if (r && r.event_id != null) { var s = settlementFromRecord(r); if (s) settlementMap[String(r.event_id)] = s; }
      }); });
  ...
  function cardStyle(settlement) { if (!settlement) return ''; return settlement.correct ? ' style="border-color:#35d49a;background:linear-gradient(180deg,#10291f,#171d2d)"' : ' style="border-color:#ff7474;background:linear-gradient(180deg,#30191d,#171d2d)"'; }
  function resultMarkup(settlement) {
    if (!settlement) return ''; var actual = settlement.actual || '—';
  ...
  function render(x) {
    var target = $('oddsBuilder'); if (!target) return; var legs = x.qualified_legs || [];
    var cards = legs.map(function (l, i) {
      var sport = String(l.sport || 'football').toLowerCase(); var competition = l.competition || (sport === 'tennis' ? 'Tennis' : 'Football');
      var settlement = settlementMap[String(l.event_id || '')] || null; var sel = selectionInfo(l);
  ...
  function showError(message) { var target = $('oddsBuilder'); if (target) target.innerHTML = '<div class="empty">Accumulator data unavailable: ' + esc(message) + '</div>'; }
  function load() {
    var target = $('oddsBuilder'); if (!target) return;
    if (!target.dataset.ready) target.innerHTML = '<div class="empty">Loading Odds Builder data…</div>';
    fetch('./data/odds_builder.json?v=' + Date.now(), { cache: 'no-store' }).then(function (r) {
      if (!r.ok) throw new Error('odds_builder.json HTTP ' + r.status); return r.json();
    }).then(function (data) {
      render(data); target.dataset.ready = '1';
      loadSettlements().then(function () { render(data); }).catch(function () {});
    }).catch(function (e) { showError(e.message); });
  }
  function updateTimers() {
    document.querySelectorAll('#oddsBuilder .timer[data-start]').forEach(function (el) {
      var c = timerState(el.getAttribute('data-start'), settlementMap[el.getAttribute('data-event-id')]); el.className = 'timer ' + c.state;
      var label = el.querySelector('.timerLabel'); var value = el.querySelector('.timerValue'); if (label) label.textContent = c.label; if (value) value.textContent = c.text;
    });
  }
  function boot() { if (!$('oddsBuilder')) return; load(); window.setInterval(updateTimers, 1000); window.setInterval(load, 60000); }
