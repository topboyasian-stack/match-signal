...
(function () {
  ...
  function render(data) {
    ...
    var legs=Array.isArray(data.qualified_legs)?data.qualified_legs:[],settledLegs=Array.isArray(data.settled_legs)?data.settled_legs:[],sports=[];
    ...
    var cards=legs.map(function(l,i){
      var settled=effectiveSettlement(l);
      return '<article class="card"><div class="meta"><span>Leg '+(i+1)+' · '+esc(String(l.sport||'').toUpperCase())+' · '+esc(l.competition||'—')+'</span><span>Fair odds '+esc(l.model_fair_odds==null?'—':l.model_fair_odds)+'</span></div><div class="teams">'+esc(l.match||'—')+'</div><div class="pick">Selection: <b>'+esc(l.pick||'—')+'</b><span class="conf">Model '+pct(l.model_probability)+'</span></div>'+timer(l.start_time,l)+'<div class="startTime">Start: '+esc(local(l.start_time))+' <span>· your browser time</span></div><div class="section"><div class="row"><span>Market</span><b>'+esc(l.market||'—')+'</b></div><div class="row"><span>Model fair odds</span><b>'+esc(l.model_fair_odds==null?'—':l.model_fair_odds)+'</b></div><div class="row"><span>Bookmaker odds</span><b>Verify manually</b></div></div></article>';
    }).join('');
