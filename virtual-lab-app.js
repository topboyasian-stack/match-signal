...
async function renderModelLab(){
  ...
  if(artifact){
    ...
    html+='</tbody></table>';
    html+='<h3>Holdout by O/U line</h3><table><thead><tr><th>Line</th><th>N</th><th>Market Brier</th><th>eFootball shape</th><th>Participant Brier</th><th>Market Log loss</th><th>eFootball shape</th><th>Participant Log loss</th></tr></thead><tbody>';
    Object.entries(artifact.by_line||{}).forEach(([line,x])=>{const a=x.holdout?.market,ef=x.holdout?.efootball_shape,b=x.holdout?.participant_model;html+='<tr><td>'+esc(line)+'</td><td>'+((ef&&ef.n)||'—')+'</td><td>'+fmt(a&&a.brier)+'</td><td>'+fmt(ef&&ef.brier)+'</td><td>'+fmt(b&&b.brier)+'</td><td>'+fmt(a&&a.log_loss)+'</td><td>'+fmt(ef&&ef.log_loss)+'</td><td>'+fmt(b&&b.log_loss)+'</td></tr>';});
    html+='</tbody></table>';
...
function renderNearQualifiedRadar(){
  const host=$('nearQualifiedRadar');if(!host)return;
  const count=$('nearQualifiedCount');
  const product=$('product').value;
  const rawComps=state.eligibility.raw_eligible_competitions||[];
  const modelComps=state.eligibility.model_qualified_competitions||[];
  const rawLines=state.eligibility.raw_eligible_ou_lines||[];
  const modelLines=state.eligibility.model_qualified_ou_lines||[];
  ...
  for(const e of upcoming){
    ...
    for(const c of markets){
      const line=Number(c.market.line);
      if(!Number.isFinite(line))continue;
      const rawLine=rawLines.some(x=>Math.abs(Number(x)-line)<0.001);
      const modelLine=modelLines.some(x=>Math.abs(Number(x)-line)<0.001);
      let modelProb=c.fairProb;
      if(ladder){
        const pseudo={
          product:e.product,
          participant_1:participantIdentity(e.participant_1||e.home||''),
          participant_2:participantIdentity(e.participant_2||e.away||''),
          home:participantIdentity(e.participant_1||e.home||''),
          away:participantIdentity(e.participant_2||e.away||''),
          timestamp:e.start_time,ladder,total:null,_livePrediction:true
        };
        const mm=modelOverForEvent(pseudo,state.modelEvents||[],line,false);
        if(mm)modelProb=String(c.pickCode).toUpperCase().startsWith('U')?1-mm.prob:mm.prob;
      }
      const cal=historicalCalibration(e.product,'ou',c.pickCode,modelProb,e.start_time);
      const edge=Number(cal.prob)-Number(c.bookImplied);
      const compName=String(e.competition||e.tournament||'');
      const compModel=modelComps.some(x=>competitionKey(x)===competitionKey(compName));
      const reasons=[];
      if(!compModel)reasons.push('LEAGUE MODEL GATE');
      if(!rawLine)reasons.push('LINE NOT RAW-ELIGIBLE');
      if(rawLine&&!modelLine)reasons.push('LINE MODEL GATE');
      if(cal.n<30)reasons.push('CALIBRATION '+cal.n+'/30');
      if(edge<0.02)reasons.push('EDGE GAP '+fmtPct(Math.max(0,0.02-edge)));
      // Radar keeps only candidates near the promotion edge: no deep-negative
      // market edges and no arbitrary fixture dump.
      if(edge<-0.005)continue;
      candidates.push({
        event:e,market:c,line,modelProb,calibratedProb:cal.prob,edge,calibrationN:cal.n,
        rawLine,modelLine,compModel,reasons
      });
    }
  }
  candidates.sort((a,b)=>(b.edge-a.edge)||(b.calibrationN-a.calibrationN)||(b.calibratedProb-a.calibratedProb));
  const top=candidates.slice(0,10);
  if(count)count.textContent=String(top.length);
  if(!top.length){
    host.innerHTML='<div class="radarEmpty"><strong>No near-qualified candidate right now.</strong><span>The radar is watching raw-evidence leagues and current O/U markets without promoting them into the prediction desk.</span></div>';
    return;
  }
  host.innerHTML=top.map((x,i)=>{
    const e=x.event,c=x.market,book=Number(c.bookmakerOdds),edge=x.edge;
    const status=edge>=0.02?'EDGE PASSED · PROMOTION BLOCKER':'CLOSE TO EDGE GATE';
    return '<article class="radarCard">'+
      '<div class="radarTop"><span>#'+(i+1)+' · '+esc(e.product)+' · '+esc(e.competition||e.tournament||'')+'</span><time>'+esc(e.start_time?date(e.start_time):'—')+'</time></div>'+
      '<div class="radarMatch"><strong>'+esc(e.home||e.participant_1||'—')+'</strong><span>vs</span><strong>'+esc(e.away||e.participant_2||'—')+'</strong></div>'+
      '<div class="radarMarket"><span>O/U '+esc(x.line)+'</span><b>'+esc(c.pickCode||'—')+'</b><span>Book '+(Number.isFinite(book)?book.toFixed(2):'—')+'</span></div>'+
      '<div class="radarMetrics"><div><small>Model</small><b>'+fmtPct(x.calibratedProb)+'</b></div><div><small>Fair</small><b>'+fmtPct(c.fairProb)+'</b></div><div><small>Edge</small><b class="'+(edge>=0.02?'positive':'warning')+'">'+(edge>=0?'+':'')+fmtPct(edge)+'</b></div><div><small>Cal n</small><b>'+x.calibrationN+'</b></div></div>'+
      '<div class="radarStatus '+(edge>=0.02?'radarReady':'')+'"><span>'+status+'</span><small>'+esc(x.reasons.slice(0,3).join(' · ')||'No major blocker identified; awaiting promotion state.')+'</small></div>'+
      '</article>';
  }).join('');
}
...
async function loadEligibility(){
  try{
    ...
    if(Array.isArray(d.model_qualified_competitions))state.eligibility.model_qualified_competitions=d.model_qualified_competitions;
    if(Array.isArray(d.model_qualified_ou_lines))state.eligibility.model_qualified_ou_lines=d.model_qualified_ou_lines.map(Number);
